...
DNAME VARCHAR2(40), LOC VARCHAR2(50)) ;

SELECT table_name  FROM  USER_TABLES;

/* #1. 테이블 생성 시 컬럼 레벨 제약조건 추가 (NOT NULL은 다른 곳에서 변경 불가)*/
CREATE TABLE person(
...
-- 2.UNIQUE 제약조건 (지금 코드는 그냥 문법 익히는 예제)
CREATE TABLE person(
...
ALTER TABLE person
  DISABLE CONSTRAINT person_name_pk CASCADE --전파(상속, 종속), PK가 있을 때 참조들도 다 비활성화
...
-- 서브쿼리를 이용한 테이블 생성(복사), 제약조건은 NOT NULL 빼고 복사가 되지 않는다.
